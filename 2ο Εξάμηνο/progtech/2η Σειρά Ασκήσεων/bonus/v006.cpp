class v006 : public Player {
    public:
        v006(const std::string& n) : Player(n) { }
    ...
    static int randomNum(int min, int max) {
        ...
        Move play(const State& s) override { // to override einai perisseio
            ...
            int mostAmountOfPossibleMoves = 0;

            for (int i = 0; i < s.getHeaps(); i++) {
                if (s.getCoins(i) > 0) {
                    leastAmountOfPossibleMoves++;
                    mostAmountOfPossibleMoves += s.getCoins(i);
                }
            }

            //We want leastAmountOfPossible moves to be as close to s.getPlayers() after our move

            if (leastAmountOfPossibleMoves > s.getPlayers() || mostAmountOfPossibleMoves <= s.getPlayers()) { //<= since it is calculated before our move so after our move it will be <
                std::vector<int> maxHeaps;
                //Bring the amount down by removing the most amount of coins possible
                for (int i = 0; i < s.getHeaps(); i++) {
                    if ((maxHeaps.size() == 0 || s.getCoins(i) >= s.getCoins(maxHeaps[0])) && s.getCoins(i) > 0) {
                        if (maxHeaps.size() > 0 && s.getCoins(i) > maxHeaps[0]) {
                            maxHeaps.clear();

                        }
                        maxHeaps.push_back(i);
                    }
                }

                if (maxHeaps.size() > 0) { //redundant because of the check from canWin() yet good to have just in case
                    source = maxHeaps[randomNum(0, maxHeaps.size() - 1)];

                    if (mostAmountOfPossibleMoves > s.getPlayers()) {
                        int amountToTake = s.getCoins(source);
                        int dif = mostAmountOfPossibleMoves - leastAmountOfPossibleMoves;

                        while (amountToTake > dif && amountToTake > 1) { //try to match LAOPM and MAOPM
                                amountToTake--;
                        }

                        return Move(source, amountToTake, 0, 0); //Removes the largest amount of coins possible, that guarantees to not instantly lose
                    }
                ...
                for (int i = 0; i < s.getHeaps(); i++) {
                    if ((maxHeaps.size() == 0 || s.getCoins(i) >= s.getCoins(maxHeaps[0])) && s.getCoins(i) > 1) {
                        if (maxHeaps.size() > 0 && s.getCoins(i) > maxHeaps[0]) {
                            maxHeaps.clear();
                ...
                if (maxHeaps.size() > 0) { //at least one heap has more than 1 coin
                    source = maxHeaps[randomNum(0, maxHeaps.size() - 1)];
    ...
    private:
        std::string type = std::string("v006");
};
