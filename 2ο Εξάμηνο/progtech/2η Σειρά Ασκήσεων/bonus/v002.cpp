...
class v002 : public Player {
    public:
        v002(const std::string& n) : Player(n) { }
            ...
            if (leastAmountOfPossibleMoves > s.getPlayers()) {
                ...
                for (int i = 0; i < s.getHeaps(); i++) {
                    if (s.getCoins(i) > 1) {
                        ...
                        if (s.getCoins(i) > 2) {
                            int count = leastAmountOfPossibleMoves;

                            for (int i = 0; i < s.getHeaps(); i++) { //combines the logic of v000 and v001 as v001 on its own proved inefficient
                                if (s.getCoins(i) > 1) {
                                    count--;
                                }
    ...
    private:
        std::string type = std::string("\033[1;34mmatthew28z-v0.0.2\033[0m");
};
